Разбор логов Nginx в формате JSON с помощью команды управления (management command).
...
Переименуйте файл `docker/prod/.env.example` в `.env` и заполните переменные окружения.
